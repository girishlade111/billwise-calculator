    ...
    <section className="relative overflow-hidden min-h-[90vh] flex flex-col bg-black text-white">
      ...
      <div className="relative z-10 container mx-auto px-4 pt-5">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
          ...
          <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
            <Button variant="ghost" size="icon" onClick={onToggleDark} className="text-white/50 hover:text-white hover:bg-white/10 rounded-lg" aria-label="Toggle theme">
      ...
      <div ref={ref} className="relative z-10 flex-1 container mx-auto px-4 flex items-center justify-center pb-20">
        <motion.div initial={{ opacity: 0, y: 25 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="max-w-2xl mx-auto text-center">
          ...
          <motion.p initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.4 }} className="text-sm text-white/40 mb-5 max-w-md mx-auto">
          ...
          <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.65 }} className="flex flex-col sm:flex-row items-center justify-center gap-2.5">
            ...
            <Button variant="ghost" className="text-white/60 hover:text-white text-sm px-4 py-5 rounded-lg" onClick={() => document.getElementById("faq")?.scrollIntoView({ behavior: "smooth" })}>
              How it Works
            </Button>
          </motion.div>

          <motion.p initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.75 }} className="mt-5 text-xs text-white/30">
            Trusted by <span className="text-white font-semibold">50,000+</span> households in India
          </motion.p>
        </motion.div>
      </div>

      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, y: [0, 8, 0] }} transition={{ delay: 1.2, duration: 2, repeat: Infinity }} className="absolute bottom-5 left-1/2 -translate-x-1/2 text-white/20">
