      ...
      <div ref={ref} className="container mx-auto px-3 relative">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4 }} className="max-w-lg mx-auto">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-center mb-6">
            ...
            <p className="text-sm text-white/70">Select state, enter units, get instant estimate</p>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white/5 border border-white/20 rounded-xl p-4 space-y-4">
            <div className="space-y-2" data-error={stateError ? "" : undefined}>
              ...
              {stateError && <p className="text-xs text-white/90 flex items-center gap-1"><Info className="w-3 h-3" />{errors.state}</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {categories.map((c) => {
                  const Icon = c.icon;
                  return (
                    <button key={c.value} onClick={() => c.available && setCategory(c.value)} className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${category === c.value && c.available ? "bg-white text-black" : "bg-white/10 border border-white/20"}`}>
            ...
            <div className="space-y-3" data-error={unitsError ? "" : undefined}>
              ...
              <div className="relative">
                ...
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-white/60">kWh</span>
              </div>
              {unitsError && <p className="text-xs text-white/90 flex items-center gap-1"><Info className="w-3 h-3" />{errors.units}</p>}
              ...
              <div className="flex justify-between text-[10px] text-white/50"><span>0</span><span>500</span><span>1000+</span></div>
            </div>
            ...
            <Button size="lg" onClick={handleCalculate} disabled={loading} className="w-full h-11 text-sm font-bold rounded-lg bg-white text-black hover:bg-white/90">
